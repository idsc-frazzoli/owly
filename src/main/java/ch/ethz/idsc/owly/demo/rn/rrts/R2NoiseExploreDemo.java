...
enum R2NoiseExploreDemo {
  ...
  public static void main(String[] args) {
    ...
    SamplerInterface spaceSampler = new CircleSampler(Tensors.vector(2, 0), RealScalar.of(3));
    SamplerInterface goalSampler = new CircleSampler(Tensors.vector(4.5, 0), RealScalar.of(.5));
    ...
    RrtsNodes.costConsistency(root, rnts, LengthCostFunction.IDENTITY);
    {
      if (rrtsPlanner.getBest().isPresent()) {
        ...
        List<RrtsNode> list = Nodes.listFromRoot(best);
        for (RrtsNode node : list)
