...
public class QuadTree<T> implements Serializable {
  ...
  public Collection<T> getElliptical(final double x1, final double y1, final double x2, final double y2, final double distance) {
    if (Math.pow(distance, 2) < Math.pow((x1 - x2), 2) + Math.pow((y1 - y2), 2)) {
      throw new IllegalArgumentException("wrong ellipse specification: distance must be greater than distance between foci." + " x1=" + x1 + " y1=" + y1
  ...
  protected static class Node<T> implements Serializable {
    ...
    public boolean remove(final double x, final double y, final T value) {
      if (this.hasChilds)
        return getChild(x, y).remove(x, y, value);
    ...
    /* default */ Collection<T> getElliptical(final double x1, final double y1, final double x2, final double y2, final double maxDistance,
      ...
      // no more childs, so we must contain the closest object
      if (this.leaf != null && this.leaf.values.size() > 0) {
        final double distance1 = Math.sqrt((this.leaf.x - x1) * (this.leaf.x - x1) + (this.leaf.y - y1) * (this.leaf.y - y1));
        // same trick as above, though it should not be useul in the vast
        // majority of cases
        if (distance1 <= maxDistance) {
          final double distance2 = Math.sqrt((this.leaf.x - x2) * (this.leaf.x - x2) + (this.leaf.y - y2) * (this.leaf.y - y2));
    ...
    /* default */ Collection<T> get(final double x, final double y, final double r_min, final double r_max, Collection<T> values) {
      if (this.hasChilds) {
    ...
    /* default */ int execute(final Rect globalBounds, final Executor<T> executor) {
      ...
      if (this.leaf != null && this.leaf.values.size() > 0 && globalBounds.contains(this.leaf.x, this.leaf.y)) {
        ...
        for (T object : this.leaf.values)
          executor.execute(this.leaf.x, this.leaf.y, object);
      }
      return count;
    }
