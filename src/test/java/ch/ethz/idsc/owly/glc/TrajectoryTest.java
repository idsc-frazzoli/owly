  ...
  public void testDisjoint() {
    TrajectoryRegionQuery goalQuery = //
        new SimpleTrajectoryRegionQuery(new TimeInvariantRegion( //
            new RnSphericalRegion(Tensors.vectorLong(10, 5), RealScalar.of(1))));
    Trajectory trajectory = new Trajectory();
    trajectory.add(new StateTime(Tensors.vectorLong(0, 5), 0));
    trajectory.add(new StateTime(Tensors.vectorLong(5, 5), 0));
    assertEquals(goalQuery.firstMember(trajectory), -1);
    assertTrue(goalQuery.isDisjoint(trajectory));
    // ---
    trajectory.add(new StateTime(Tensors.vectorLong(10, 5), 0));
